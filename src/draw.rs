    ...
    match session.mode {
        ...
        Mode::Visual(VisualState::LookupSampling) => {            
            if let Some(res) = &session.lookup_result {
                ...
                let z = v.zoom;
                for i in 0..v.animation.len() {
                    let frame_offset = (i as u32 * v.fw) as f32;

                    for &(ox, oy) in &res.other_queries {
                        let r = ox as f32 + frame_offset;
                        let g = oy as f32;
                        let p = SessionCoords::new(
                        ...
                        shapes.add(Shape::Rectangle(
                            ...
                            self::UI_LAYER,
                            Rotation::ZERO,
                            Stroke::new(1.0, color::YELLOW.into()),
                            Fill::Empty,
                        ));

                        if let Some((mini_id, mini_view)) = &session.miniview {
                            if *mini_id == res.view_id {
                    ...
                    let r = res.cursor_x as f32 + frame_offset;
                    let g = res.cursor_y as f32;
                    let p = SessionCoords::new(
                        session.offset.x + v.offset.x + r * z,
                        session.offset.y + v.offset.y + (v.fh as f32 - 1.0 - g) * z,
                    );
...
pub fn draw_view_lookuptexture_animation<R>(v: &View<R>) -> sprite2d::Batch {
    sprite2d::Batch::singleton(
        v.width(), // source is layerdata.fb
        v.fh,
        Rect::origin(v.width() as f32, v.fh as f32),
        Rect::origin(v.width() as f32, v.fh as f32),
        self::VIEW_LAYER,
        Rgba::TRANSPARENT,
        1.,
...
pub fn draw_view_lookuptexture_fb<R>(session: &Session, v: &View<R>) -> sprite2d::Batch {
    sprite2d::Batch::singleton(
        v.width(), // source is layerdata.lookup_anim_fb (full spritesheet)
        v.fh,
        *v.animation.val(),
        Rect::new(
            -(v.fw as f32) * 2.,
